...
function saveColors() {
  ...
  for (let angle = 0; angle < 360; angle++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      writer.print("pixel: " + i);
      writer.print(" r: " + red(pixel));
      writer.print(" g: " + green(pixel));
      writer.print(" b: " + blue(pixel));
    }
  }
  writer.close();
  writer.clear();
...
function drawPicture(drawAngle) {
  loadPixels();
  for (let i = 0; i < len; i += gap + 1) {
    currPixel.set(center);
    direction.set(p5.Vector.fromAngle(radians(drawAngle), i));
    currPixel.add(direction);

    set(
      int(currPixel.x) + width / 2 - center.x,
      int(currPixel.y) + height / 2 - center.y,
...
function setLen() {
  ...
  if (val == "" || val > min(pic.width, pic.height) / 2) {
